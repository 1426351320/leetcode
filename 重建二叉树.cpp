...
TreeNode * Solution::reConstructBinaryTree(vector<int> pre, vector<int> vin)
{
	...
	else
	{
		...
		NewNode->left = reConstructBinaryTree(vector<int>(pre.begin() + 1, pre.begin() + 1 + LeftNum), vector<int>(vin.begin(), HeadPos));
		NewNode->right = reConstructBinaryTree(vector<int>(pre.begin() + 1 + LeftNum, pre.end()), vector<int>(HeadPos + 1, vin.end()));;
		return NewNode;
	}
}
